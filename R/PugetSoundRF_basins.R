...
original <- ggplot(data = data, aes(x=Date, y=RF_Count)) + 
...
overall <-  ggplot(data = data, aes(x=Date, y=RF_Count)) + 
  geom_point(color="gray30", size=2, aes(shape="Survey")) + theme_cowplot() + 
  theme(axis.title.x=element_blank()) + ylab("Count (total)") +
  scale_shape_manual(name="Legend", values=1) +
  scale_y_continuous(trans='log2') 
...
ggplot() + 
  ...
  theme(axis.text.x=element_text(hjust=0.9,vjust=0.8,angle=45))
#900x500
