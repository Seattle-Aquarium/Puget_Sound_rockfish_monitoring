...
dat <- dat %>% mutate(Season =
                        case_when(Month == 1 ~ "Winter", Month == 2 ~ "Winter",
...
#wide to long format (make tidy)
long.dat <- dat %>%
  pivot_longer("Black":"Puget Sound", names_to = "Species", values_to = "Count")

#add Count (Adult rf only) and Shannon-Weiner Diversity Index values as columns

long.dat <- filter(long.dat, Species != "YOY") #removeYOY

long.dat <- long.dat %>% 
  group_by(Site, Date) %>%
  mutate(Shannon = diversity(Count, "shannon")) #Add Shannon column
...
ggboxplot(ARFdat, x="Season", y = "Abundance", color = "Season") + 
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#THIS NEXT SECTION WORKS WITH SHANNON DIVERSITY INDEX

#I want to add a column to dat with SWDI, based on values of adult fish
#step one, remove YOY; step 2 add column with calculation
...
dat <- long.dat %>% 
  ...
  unique()#create database with just one row per site w/Shannon info
...
ggboxplot(dat, x="Site", y = "Shannon") + 
...
#run stats
res.aov2 <- aov(Shannon ~ Site + Season, data = dat)
summary(res.aov2)

res.aov3 <- aov(Shannon ~ Site * Season, data = dat)
summary(res.aov3)

sumstats <- group_by(dat, Site, Season) %>%
  summarise(
    count = n(),
...
TukeyHSD(res.aov3, which = "Season")
#there may be no difference between spring vs. winter AND summer vs. fall

#passed test for normalcy
