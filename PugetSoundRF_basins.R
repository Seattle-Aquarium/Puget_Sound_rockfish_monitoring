...
## This script includes code used for analyzing basin differences for the 2009-2023 puget sound rockfish data. Run after running "PugetSoundRF_datacleaning.R" code.
...
comm <- ggplot(data, aes(x=Species, y=Count, shape=Location, color=Location)) + geom_point(alpha=0.75, size=2.5) + 
  facet_wrap(~Location) + theme_cowplot() + coord_flip() + 
...
overall <- ggplot(data = data, aes(x=Date, y=RF_Count)) + 
  geom_point(color="gray30", size=2, shape=1) + geom_smooth(method=glm, se=FALSE, aes(color="All sites, species")) + theme_cowplot() + 
  annotate("text", x=as.Date("2012-05-02"), y=256, label="Tau = -0.139") +
...
basin <- ggplot(data = data, aes(x=Date, y=RF_Count, group=Location, shape=Location, color=Location)) +
  ...
  annotate("text", x=as.Date("2013-10-09"), y=400, label="Tau=-0.239;", color="gray40") + 
  annotate("text", x=as.Date("2013-10-09"), y=250, label="p=0.053", color="gray40") +
  annotate("text", x=as.Date("2012-05-02"), y=60, label="Tau=-0.414;") +
  annotate("text", x=as.Date("2012-05-02"), y=40, label="p<0.001") +
  annotate("text", x=as.Date("2010-05-19"), y=7, label = "Tau=0.019", color="gray60") +
  annotate("text", x=as.Date("2010-05-19"), y=5, label = "p=0.842", color = "gray60") +
  annotate("text", x=as.Date("2012-11-02"), y=5, label ="Tau=0.667", color="gray20") +
  annotate("text", x=as.Date("2012-11-02"), y=3, label = "p=0.118", color="gray20") +
...
Hood <- fig.dat %>% filter(Location == "Hood Canal" & Species == "Copper")
cor <- cor.test(Hood$Year, Hood$Count, method = "kendall")
print(cor)

Admiralty <- fig.dat %>% filter(Location == "Admiralty Inlet" & Species == "Black")
cor <- cor.test(Admiralty$Year, Admiralty$Count, method = "kendall")
print(cor)

Central <- fig.dat %>% filter(Location == "Central Puget Sound" & Species == "Black")
...
ggarrange(overall, basin, HC, labels = c("A", "B", "C"))
#900wx800h
...
plot.ord <- function(data, Admiralty, Central, Hood, South){
  t2 <- ggplot(data, aes(x = NMDS1, y = NMDS2)) + my.theme + coord_fixed() + 
    ...
    annotate("text", x=1.5, y=1, label="Pr(>F)<0.001")
  return(t2)
}
...
ggarrange(comm, p2, nrow=1, labels=c("A", "B"))
#1175x450
